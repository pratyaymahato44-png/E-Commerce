...
export async function clerkWebHookHandler(req: Request, res: Response) {
    ...
    try {
        ...
        if (evt.type === "user.created" || evt.type === "user.updated") {
            ...
            const primaryEmail = u.email_addresses?.find((email) => email.id === u.primary_email_address_id);

            const email = primaryEmail?.email_address || u.email_addresses?.[0]?.email_address
